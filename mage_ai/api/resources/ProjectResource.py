...
async def build_project(
    ...
    **kwargs,
    ...
    ai_config = project.repo_config.ai_config or {}
    model = merge_dict(project.repo_config.to_dict(), dict(
        ai_mode=ai_config.get('mode') if isinstance(ai_config, dict) else None,
...
class ProjectResource(GenericResource):
    ...
    async def update(self, payload, **kwargs):
        ...
        if 'openai_base_url' in payload:
            ...
            if repo_config.openai_base_url != openai_base_url:
                data['openai_base_url'] = openai_base_url

        if 'openai_model' in payload:
            openai_model = payload.get('openai_model')
            if repo_config.openai_model != openai_model:
                data['openai_model'] = openai_model

        if 'ai_mode' in payload:
            ai_config = dict(repo_config.ai_config or {})
            ai_config['mode'] = payload.get('ai_mode')
